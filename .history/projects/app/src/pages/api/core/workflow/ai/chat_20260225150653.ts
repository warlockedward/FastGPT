...
async function handler(req: NextApiRequest, res: NextApiResponse): Promise<AiChatResponseType> {
  ...
  try {
    ...
    await addMessage(currentSession.sessionId, {
      role: 'assistant',
      content: result.message || '工作流已生成'
    });

    // Update workflow state if present
    if (result.workflow) {
      await updateWorkflowNodes(currentSession.sessionId, {
        nodes: result.workflow.nodes,
        edges: result.workflow.edges
      });
    }

    // Update validation status if present
    if (result.validation_issues) {
      await setWorkflowValidation(currentSession.sessionId, {
        isValid: result.validation_issues.length === 0,
        validationErrors: result.validation_issues.map((i: any) => i.message)
      });
    }
