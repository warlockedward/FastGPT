...
async function handler(req: NextApiRequest, res: NextApiResponse): Promise<any> {
  ...
  console.log('Handler body:', { message, sessionId, context, stream });
  ...
  // Get or create/* get session */
  console.log('Before getSession, sessionId:', sessionId);
  let currentSession = sessionId ? await getSession(sessionId) : null;
  console.log('After getSession, currentSession:', currentSession);
