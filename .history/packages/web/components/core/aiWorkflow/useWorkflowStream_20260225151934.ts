...
export function useWorkflowStream({
  ...
  // Refs for callbacks to ensure latest version is used in async loop
  const onSessionIdRef = useRef(onSessionId);
  onSessionIdRef.current = onSessionId;
  const onIntentDetectedRef = useRef(onIntentDetected);
  onIntentDetectedRef.current = onIntentDetected;
  const onNodeGeneratingRef = useRef(onNodeGenerating);
  onNodeGeneratingRef.current = onNodeGenerating;
  const onNodeGeneratedRef = useRef(onNodeGenerated);
  onNodeGeneratedRef.current = onNodeGenerated;
  const onValidationProgressRef = useRef(onValidationProgress);
  onValidationProgressRef.current = onValidationProgress;
        ...
        for (const line of lines) {
          ...
          switch (type) {
            case 'aiWorkflowSession':
              if (data.sessionId) {
                onSessionIdRef.current?.(data.sessionId);
              }
              break;
            ...
            case 'aiWorkflowIntent':
              ...
              if (data.intent && data.complexity) {
                onIntentDetectedRef.current?.(data.intent, data.complexity);
              }
              break;
            ...
            case 'aiWorkflowNodeGenerated':
              ...
              if (data.node) {
                onNodeGeneratedRef.current?.(
                  data.node,
                  data.partialWorkflow || workflow,
                  data.progress
                );
              }
              break;
            ...
            case 'aiWorkflowValidationProgress':
              setProgress(data.progress);
              if (data.level) {
                onValidationProgressRef.current?.(data.progress, data.level);
              }
              break;
